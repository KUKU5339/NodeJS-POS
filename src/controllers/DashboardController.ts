...
export async function stats(req: Request, res: Response) {
  ...
  const productsCount = await (Product as any).count({ where: { userId } });
  const sales = await (Sale as any).findAll({ where: { userId } });
...
export async function recentSales(req: Request, res: Response) {
  const userId = (req as any).userId || 1;
  const sales = await (Sale as any).findAll({ where: { userId }, order: [['id', 'DESC']], limit: 10 });
...
export async function lowStock(req: Request, res: Response) {
  const userId = (req as any).userId || 1;
  const products = await (Product as any).findAll({ where: { userId }, order: [['stock', 'ASC']], limit: 5 });
  res.json({
...
export async function topProducts(req: Request, res: Response) {
  ...
  const topIds = Object.entries(counts)
    ...
    .slice(0, 5);
  const products = topIds.length ? await (Product as any).findAll({ where: { id: { [Op.in]: topIds.map(i => i.id) } } }) : [];
  const nameMap = new Map(products.map(p => [p.id, p.name]));
